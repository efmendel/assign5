...
class TestUserProfile:
    def test_valid_user_profile(self):
        valid_user_path = (
            Path(__file__).parent.parent / "data" / "valid" / "input" / "user.json"
        )
    ...
    def test_invalid_user_profile(self):
        invalid_user_path = (
            Path(__file__).parent.parent / "data" / "invalid" / "user.json"
        )
    ...
    def test_valid_name(self):
        ...
        assert (
            UserProfile.valid_name("A B") == True
        )  # Single uppercase letters are valid

        assert UserProfile.valid_name("SingleName") == False  # Only one part
        assert (
            UserProfile.valid_name("john Smith") == False
        ...
        assert (
            UserProfile.valid_name("Too Many Name Parts Extra") == False
        )  # More than 3 parts
    ...
    def test_user_serialization(self):
        ...
        valid_output_path = (
            Path(__file__).parent.parent / "data" / "valid" / "output" / "user.json"
        )
        temp_output_path = (
            Path(__file__).parent.parent / "data" / "temp_user_output.json"
        )
